...
st.set_page_config(page_title="Seleção de Elenco", layout="wide")
...
def format_func(row):
    return f"{row['Name']} - OV: {row['overall']} - €{row['Market Value (M€)']}M"

st.title("⚽ Seleção de Elenco Profissional")
...
if uploaded_file is None:
    try:
        ...
        st.info("Aguardando upload da planilha ou arquivo 'jogadores.xlsx' não encontrado.")
        st.stop()
else:
...
if data:
    ...
    if elenco:
        ...
        st.sidebar.metric("Custo Total", f"€{df_f['Market Value (M€)'].sum():.1f}M")
        st.sidebar.metric("Média Overall", f"{df_f['overall'].mean():.1f}")
        
        out = BytesIO()
        with pd.ExcelWriter(out, engine='openpyxl') as w:
            df_f.to_excel(w, index=False)
        st.sidebar.download_button("💾 Baixar Time", out.getvalue(), f"{nome_time}.xlsx")
