...
if st.button("🚀 ENVIAR AGORA", type="primary", use_container_width=True):
    if not (t1 and t2):
        ...
        with st.status("Iniciando envio...", expanded=True) as status:
            try:
                # 1. Preparar Texto (IDs e Nomes)
                body = f"TIME: {time_nome}\nTECNICOS: {t1} & {t2}\n\nJOGADORES:\n"
                ...
                pdf.multi_cell(0, 10, txt=body.encode('latin-1', 'replace').decode('latin-1'))
                pdf_bytes = pdf.output(dest='S').encode('latin-1', 'ignore')

                # 3. Email
                msg = MIMEMultipart()
                msg['Subject'] = f"Inscrição PES: {time_nome}"
                msg['From'] = EMAIL_REMETENTE
                msg['To'] = EMAIL_DESTINO
                msg.attach(MIMEText(body, 'plain'))

                # Anexo TXT (Apenas IDs e Nomes)
                ...
                att_pdf.add_header('Content-Disposition', f'attachment; filename="Resumo.pdf"')
                msg.attach(att_pdf)

                status.update(label="Enviando para o servidor...", state="running")
                with smtplib.SMTP_SSL('smtp.gmail.com', 465) as s:
                    s.login(EMAIL_REMETENTE, SENHA_APP)
                    s.send_message(msg)
                
                status.update(label="✅ Inscrição enviada com sucesso!", state="complete")
                st.balloons()
