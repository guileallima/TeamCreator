...
with st.sidebar:
    ...
    st.divider()
    qtd = len([p for p in st.session_state.squad.values() if p])
    st.metric("Saldo", f"€{saldo:.0f}")
    st.write(f"Elenco: {qtd}/16")
